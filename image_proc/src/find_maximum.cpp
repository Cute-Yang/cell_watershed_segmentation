...
float true_edm_height(int x, int y, const ImageMat<float>& input_mat) {
    ...
    constexpr float half_sqrt2 = 0.5f * sqrt_2;
    if (x == width - 1 || y == height - 1 || x == 0 || y == 0 || v == 0.0f) {
        return v;
    } else {
        float true_h       = v + half_sqrt2;
...
void get_sorted_max_points(const ImageMat<float>& input_mat, ImageMat<uint8_t>& type_mat,
                           float global_min, float global_max, float threshold,
                           bool exclude_edge_now, bool is_EDM,
                           std::vector<ValueWithCoordinate>& max_record_points) {
    // this is very important! be sure the data is clean
    type_mat.set_zero();
    bool check_threshold = (threshold != NO_THRESHOLD);
    int  height          = input_mat.get_height();
    int  width           = input_mat.get_width();
    // to handle the large image!
    size_t n_max = 0;
        ...
        for (int x = 0; x < width; ++x) {
            ...
            for (int d = 0; d < 8; ++d) {
                if (is_inner || is_within(x, y, d, height, width)) {
                    ...
                    if (is_EDM) {
                        value_neightbor_true = true_edm_height(
                            x + DIRECTION_OFFSETS[d].x, y + DIRECTION_OFFSETS[d].y, input_mat);
                    }
            ...
            if (is_max) {
                type_mat(y, x) = Constant::MAXIMUM;
                ++n_max;
            }
        }
    }
    // auto  type_view    = type_mat.get_image_view();
    float value_factor = 2e9 / (global_max - global_min);
    max_record_points.resize(n_max);
    // std::vector<ValueWithCoordinate> max_record_points(n_max);
    size_t p_idx = 0;
    // optimize
    if (is_EDM) FISH_LIKELY_STD {
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    if (type_mat(y, x) == Constant::MAXIMUM) {
                        float value_f32 = true_edm_height(x, y, input_mat);
                        int   value_i32 = static_cast<int>((value_f32 - global_min) * value_factor);
                        // using at replace the index access
                        // max_record_points.at(p_idx) = {value_i32, x, y};
    ...
    // sort by value,if the value equal,sort by index,the bit order from high to low is value y x!
    auto sort_func = [](const ValueWithCoordinate& lhs, const ValueWithCoordinate& rhs) {
    ...
    std::sort(max_record_points.begin(), max_record_points.end(), sort_func);
    // sort array by value!
    LOG_INFO("sorting the record points...");
}
...
void analyze_and_mark_maxima(const ImageMat<float>& input_mat, ImageMat<uint8_t>& type_mat,
    ...
    int                       height      = input_mat.get_height();
    int                       width       = input_mat.get_width();
    size_t                    element_num = static_cast<size_t>(height) * width;
    std::vector<Coordinate2d> point_coordinates(element_num);
    ...
    for (int i_max = n_max - 1; i_max >= 0; --i_max) {
        ...
        float value_0 = is_EDM ? true_edm_height(x0, y0, input_mat) : input_mat(y0, x0);
        bool  sorting_error;
        while (true) {
            // just for tmep!
            ...
            while (true) {
                ...
                for (int d = 0; d < 8; ++d) {
                    ...
                    if ((is_inner || is_within(x, y, d, height, width)) &&
                        ...
                        if (is_EDM && input_mat(y_plus_offset, x_plus_offset) <= 0.0f) {
                            continue;
                        }
                        ...
                        if (value_plus_offset > value_0 + max_sorting_error) {
                            ...
                            point_coordinates[element_num] = {x_plus_offset, y_plus_offset};
                            ++element_num;
            ...
            if (sorting_error) {
                for (int i = 0; i < element_num; ++i) {
                    // retry!
                    type_mat(point_coordinates[i].y, point_coordinates[i].x) = 0;
                }
            } else {
                int reset_mask =
                ...
                for (int i = 0; i < element_num; ++i) {
                    ...
                    type_mat(y, x) &= reset_mask;
                    // means that we already processed this point!
                    type_mat(y, x) |= Constant::PROCESSED;
                    if (max_possible) {
    ...
    if (n_max == 0) {
        ...
        std::memset(type_ptr, fill_data_size, filled_value);
    };
}
...
void make_8bit_mat(const ImageMat<float>& distance_mat, const ImageMat<uint8_t>& type_mat,
    ...
    if (is_EDM) {
        ...
        min_value = (threshold == NO_THRESHOLD) ? global_min : threshold;
    }
    // this is a magic number!
    constexpr double magic_number = 1.0 / 253.0 / 2.0 - 1e-6;
    double offset = min_value - static_cast<double>(global_max - min_value) * magic_number;
    double factor = 253.0 / static_cast<double>(global_max - min_value);
    if (is_EDM) {
        // clip
        if (factor > 1.0) {
            factor = 1.0;
    ...
    if (channels != 1) {
        LOG_ERROR("the channels of distance mat should be 1 ,but got {}", channels);
        return;
    }
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            float original_value = distance_mat(y, x);
...
void cleanup_maximum(ImageMat<uint8_t>& output_mat, ImageMat<uint8_t>& type_mat,
    ...
    for (int i_max = n_max - 1; i_max >= 0; --i_max) {
        int x0 = max_points[i_max].x;
        int y0 = max_points[i_max].y;
        if ((type_mat(y0, x0) & (Constant::MAX_AREA | Constant::ELIMINATED)) != 0) {
            continue;
        }
        int level            = output_mat(y0, x0) & 255;
        int lo_level         = level + 1;
        point_coordinates[0] = {x0, y0};
        type_mat(y0, x0) |= Constant::LISTED;
        int element_num         = 1;
        int last_num            = 1;
        int current_element_idx = 0;
        // an point!
        ...
        while (!saddle_found && lo_level > 0) {
            ...
            while (true) {
                ...
                for (int d = 0; d < 8; ++d) {
                    ...
                    // attention here...
                    if ((is_inner || is_within(x, y, d, height, width)) &&
                        (type_mat(y2, x2) & Constant::LISTED) == 0) {
                        if ((type_mat(y2, x2) & Constant::MAX_AREA) != 0 ||
                            (((type_mat(y2, x2) & Constant::ELIMINATED) != 0) &&
                             (output_mat(y2, x2) & 255) >= lo_level)) {
                            // whether find the saddle point!
                            saddle_found = true;
                            break;
                        } else if ((output_mat(y2, x2) & 255) >= lo_level &&
                                   (type_mat(y2, x2) & Constant::ELIMINATED) == 0) {
                            point_coordinates[element_num] = {x2, y2};
                            ++element_num;
                            type_mat(y2, x2) |= Constant::LISTED;
                        }
                    }
                }
                // just find
        ...
        for (int i = 0; i < element_num; ++i) {
            type_mat(point_coordinates[i].y, point_coordinates[i].x) &= (~Constant::LISTED);
        }
...
int process_level(int pass, ImageMat<uint8_t>& input_mat, int level_start, int level_npoints,
    ...
    for (int i = 0, p = level_start; i < level_npoints; ++i, ++p) {
        ...
        if (x < xmax && y > 0 && input_mat(y - 1, x + 1) == 255) {
            index ^= 2;
        }
        ...
        if (x > 0 && y < ymax && input_mat(y + 1, x - 1) == 255) {
            index ^= 32;
        }
...
void watershed_segment(ImageMat<uint8_t>& input_mat) {
    ...
    //...
    size_t remove_size = histogram[0] + histogram[255];
    LOG_INFO("we will remove pixel,for value=0,remove {} value=255,remove {}",
             histogram[0],
             histogram[255]);
    size_t array_size = static_cast<size_t>(height) * static_cast<size_t>(width) - remove_size;
    ...
    // 最大不超过图片总像素的1/3
    size_t estimate_changed_point_size = FISH_MIN(max_bin_size, (width * height + 2) / 3);
    std::vector<Coordinate2d> set_points;
...
Status::ErrorCode find_maxima_impl(const ImageMat<float>& distance_mat,
    ...
    if (output_type == EDMOutputType::SEGMENTED) {
        ...
        // auto maximum_mask_view = maximum_mask.get_image_view();
        // auto type_mat_view     = type_mat.get_image_view();
