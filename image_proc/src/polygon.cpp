...
int line_circle_intersection(double xA, double yA, double xB, double yB, double xC, double yC,
    ...
    double y_C2 = len_AC * std::sin(phi3);
    // avoid got negative value in sqrt!
    if (std::abs(y_C2) > rad) {
        return 0;
    }
    ...
    size_t select_num = 0;
    // sinx^2  + cosx^2 = 1
    if ((sect_one >= 0 && sect_one <= x_B2) || !ignore_outside) {
...
PolygonTypef32 get_interpolated_polygon(const PolygonTypef32& ori_polygon, double interval,
    ...
    double side_len = compute_polygon_side_len<float, false>(polygon);
    if (interval <= 0.01) {
    ...
    while (trial <= n_trials) {
        ...
        while (src_idx < npoints - 1) {
            ...
            if (inter_num >= 1) {
                ...
                ++dst_idx;
                xA = intersections[0].x;
                yA = intersections[0].y;
        ...
        ++dst_idx;
        dst_polygon.emplace_back(polygon[npoints - 1].x, polygon[npoints - 1].y);
        ++dst_idx;
        if (!allow_to_adjust) {
            if (is_line) {
                --dst_idx;
...
void smooth_polygon_roi(const PolygonTypef32& polygon, PolygonTypef32& smoothed_polygon) {
    smoothed_polygon.clear();
    size_t vertex_size = polygon.size();
    // allocate the buffer! just use floor divede,because the stride is 2,and we not consider the
    // last point while it not divide fully1
    // for example,if the vertex size is 5,we will keep 3!
    smoothed_polygon.resize((vertex_size + 1) / 2);
    // mean poly smoth with 3 point!
    constexpr float k = 1.0 / 3.0;

    size_t smooth_idx = 0;
    for (size_t idx = 0; idx < vertex_size; idx += 2) {
        // perfect!
        int   idx_prev = (idx - 1 + vertex_size) % vertex_size;
        int   idx_next = (idx + 1) % vertex_size;
        ...
        smoothed_polygon[smooth_idx].set_coor(x, y);
        ++smooth_idx;
    }
...
void PolygonInterpolator::get_interpolated_polygon_impl(const PolygonTypef32& original_polygon,
                                                        PolygonTypef32&       out_polygon,
                                                        double interval, bool smooth,
                                                        RoiType roi_type) {
    // be sure the data is clean!
    clear_resource();
    // be sure the out data is clean!
    out_polygon.clear();
    // in qupath watershem,the roi type is traced roi,so we just do these!
    size_t original_vertex_size = original_polygon.size();
    if (original_vertex_size <= 2) {
        out_polygon.assign(polygon.begin(), polygon.end());
        return;
    }
    polygon.reserve(original_vertex_size + 1);
    polygon.resize(original_vertex_size);
    // copy the ori points...
    std::copy(original_polygon.begin(), original_polygon.end(), polygon.begin());

    bool apply_smooth = (smooth && (roi_type == RoiType::TRADED_ROI ||
                                    roi_type == RoiType::FREEROI || roi_type == RoiType::FREELINE));
    // whether apply smooth!
    if (apply_smooth) {
        constexpr float smooth_k = 1.0f / 3.0f;
        // smooth the point with 3 neigh,prev,self,and next along the clockwise !
        for (size_t i = 1; i < original_vertex_size - 2; ++i) {
            polygon[i].x = (polygon[i - 1].x + polygon[i].x + polygon[i + 1].x) * smooth_k;
            polygon[i].y = (polygon[i - 1].y + polygon[i].y + polygon[i + 1].y) * smooth_k;
        }
        if (roi_type != RoiType::FREELINE) {
            // process the first and last point! so easy so simple!
            polygon[0].x =
                (polygon[original_vertex_size - 1].x + polygon[0].x + polygon[1].x) * smooth_k;
            polygon[0].y =
                (polygon[original_vertex_size - 1].y + polygon[0].y + polygon[1].y) * smooth_k;

            polygon[original_vertex_size - 1].x =
                (polygon[original_vertex_size - 2].x + polygon[original_vertex_size - 1].x +
                 polygon[0].x) *
                smooth_k;
            polygon[original_vertex_size - 1].y =
                (polygon[original_vertex_size - 2].y + polygon[original_vertex_size - 1].y +
                 polygon[0].y) *
                smooth_k;
        }
    }
    ...
    size_t estimate_dst_vertex_size = static_cast<size_t>(10 + (side_len * 1.5) / interval);

    double try_interval  = interval;
    double min_diff      = 1e9;
    double best_interval = 0.0;
    dst_polygon.reserve(estimate_dst_vertex_size);
    ...
    while (trial <= n_trials) {
        ...
        while (src_idx < vertex_size - 1) {
            ...
            int    inter_num =
                line_circle_intersection(xA, yA, xB, yB, xC, yC, try_interval, true, intersections);
            if (inter_num >= 1) {
                // only process the first vertext!
                ++dst_idx;
                xA = intersections[0].x;
                yA = intersections[0].y;
                dst_polygon.emplace_back(intersections[0].x, intersections[0].y);
        ...
        ++dst_idx;
        dst_polygon.emplace_back(polygon[vertex_size - 1].x, polygon[vertex_size - 1].y);
        ++dst_idx;
        if (!allow_to_adjust) {
            if (is_line) {
                --dst_idx;
    ...
    // convert double to float!
    out_polygon.reserve(dst_idx);
    for (int i = 0; i < dst_idx; ++i) {
        out_polygon.emplace_back(dst_polygon[i].x, dst_polygon[i].y);
    }
}
