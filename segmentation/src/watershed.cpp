...
// the implementaion of add neighbors maybe diff with quapth!
template<class T>
FISH_ALWAYS_INLINE void add_neighbors_with_4_conn(WatershedQueueWrapper<T>& queue, int x, int y,
                                                  const ImageMat<T>& image) {
    int height = image.get_height();
    int width  = image.get_width();
    if (y > 0) FISH_LIKELY_STD {
            queue.add(x, y - 1, image(y - 1, x));
        }
    if (x > 0) FISH_LIKELY_STD {
            queue.add(x - 1, y, image(y, x - 1));
        }

    if (x < width - 1) FISH_LIKELY_STD {
            queue.add(x + 1, y, image(y, x + 1));
        }
    if (y < height - 1) FISH_LIKELY_STD {
            queue.add(x, y + 1, image(y + 1, x));
        }
}
...
void add_neighbors_with_8_conn(WatershedQueueWrapper<T>& queue, int x, int y,
    ...
    for (int yy = y_start; yy < y_end; ++yy) {
        for (int xx = x_start; xx < x_end; ++xx) {
            if (xx == x && yy == y) FISH_UNLIKELY_STD {
                    continue;
                }
...
Status::ErrorCode watershed_transform_impl(const ImageMat<T1>& image, ImageMat<T2>& marker,
    ...
    // maybe got an empty mat,so we should allocate the buffer!
