...
template<class T> struct Float32TypeRequire {
    static constexpr bool value = std::is_same_v<T, float>;
};

template<class T> struct Float64TypeRequire {
...
private:
    ...
    void check_access_index(int d0, int d1, int d2) const {
        if (d0 >= height || d1 >= width || d2 >= channels) {
            ...
            throw FishException(Status::ErrorCode::InvallidMatIndex,
                                "out of range",
                                __FILE__,
                                __FUNCTION__,
                                __LINE__);
...
public:
    ...
    // this is very important,we must specify the layout of mat!
    ImageMat()
        : height(0)
        , width(0)
        , channels(0)
    ...
    ImageMat(int height_, int width_, int channels_, T* buf_,
        ...
        if (!copy) {
            ...
            if (data_ptr == nullptr) {
                ...
                LOG_INFO("copy data from {:x} to {:x}",
    ...
    // must add noexcept! otherwise,will not trace the move semantics
    ...
    // support the const invoke!
    constexpr T get_dtype_min() const { return std::numeric_limits<T>::lowest(); }

    constexpr T get_dtype_max() const { return std::numeric_limits<T>::max(); }

    std::vector<T> get_image_view() const { return std::vector<T>(data_ptr, data_ptr + 1); }
    ...
    // if the image is very large,we should be sure we will not overflow!
    size_t get_element_num() const noexcept {
        size_t height_cast   = static_cast<size_t>(height);
        size_t width_cast    = static_cast<size_t>(width);
        size_t channels_cast = static_cast<size_t>(channels);
        size_t element_num   = height_cast * width_cast * channels_cast;
        return element_num;
    }
    ...
    void __attribute__((noinline)) print_1d_with_specify_interval(size_t left, size_t right) {
    ...
    // the function maybe cost much time QAQ
    friend std::ostream& operator<<(std::ostream& os, const ImageMat<T>& mat) {
        ...
        constexpr size_t value_size = ValuePrintSizeTraits<T>::size;
        for (int h = 0; h < display_height; ++h) {
            for (int w = 0; w < display_width; ++w) {
                for (int c = 0; c < display_channels - 1; ++c) {
                    if constexpr (CharTypeRequire<T>::value) {
                        os << std::setw(value_size) << static_cast<int>(mat.at(h, w, c)) << " ";
                    } else {
                        os << std::setw(value_size) << mat.at(h, w, c) << " ";
                    }
                }
                if constexpr (CharTypeRequire<T>::value) {
                    os << std::setw(value_size)
                       << static_cast<int>(mat.at(h, w, display_channels - 1));
                } else {
                    os << std::setw(value_size) << mat.at(h, w, display_channels - 1);
                }
                if (w < mat.width - 1) {
                    os << ",";
                }
        ...
        for (int h = height_tail_start; h < mat.height; ++h) {
            ...
            // avoid print twice for some rows...
            int tail_width_start = FISH_MAX(display_width, mat.width - display_width);
            for (int w = tail_width_start; w < mat.width; ++w) {
                for (int c = 0; c < display_channels - 1; ++c) {
                    if constexpr (CharTypeRequire<T>::value) {
...
Status::ErrorCode convert_mat(const ImageMat<T1>& input_mat, ImageMat<T2>& output_mat) {
    ...
    LOG_INFO("convert type from type {} to type {}",
             ValuePrintNmaeTraits<T1>::name,
             ValuePrintNmaeTraits<T2>::name);
...
template<class T> struct GenericCoordinate2d {
    ...
    // generic coordinate should suppoer the negaitive value!
    GenericCoordinate2d()
        : x(zero_value)
        , y(zero_value) {}
    GenericCoordinate2d(T x_, T y_)
        : x(x_)
        , y(y_) {}
    ...
    void set_coor(T x_, T y_) noexcept {
        x = x_;
        y = y_;
    }
    ...
    GenericCoordinate2d& operator+(const GenericCoordinate2d<T>& rhs) {
        return GenericCoordinate2d(x + rhs.x, y + rhs.y);
    }
    GenericCoordinate2d& operator-(const GenericCoordinate2d<T>& rhs) {
        return GenericCoordinate2d(x - rhs.x, y - rhs.y);
    }
